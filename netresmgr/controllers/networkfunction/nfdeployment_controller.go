...
func (r *NfDeploymentReconciler) processNfCommonPkg(ctx context.Context, name string, nfClassName string, nfType string, objectNamespace string, NfNamespace string, pr *porchv1alpha1.PackageRevision) error {
	...
	for _, site := range spec.Sites {
		...
		if newPR, err := porchutil.ClonePackage(ctx, r.PorchClient, srcPkgName, newPkgName, catalogRepo, objNamespace); err != nil {
			...
			if err := r.processNfCommonPkg(ctx, site.Id, site.NfClassName, site.NfKind, objNamespace,
				site.NfNamespace, newPR); err != nil {
				return err
			}
		}
